function post_edit(tweet_id) {
    tweet_content = document.querySelector('#post_content'+tweet_id).value
    fetch ('/edit', {
        method: 'POST',
        body: JSON.stringify({
    ...
    document.querySelector('#body'+tweet_id).innerHTML=tweet_content;
...
function edit(tweet__id){
    ...
    document.querySelector('#divedit'+tweet__id).innerHTML='<p></p><div class="form-group"><textarea id="post_content'+tweet__id+'" name="post-content" cols="30" rows="3" class="form-control">' + body + '</textarea><p></p><input onclick="post_edit('+tweet__id+')" type="Submit" value="Update" class="btn btn-primary"><p></p></div>';
